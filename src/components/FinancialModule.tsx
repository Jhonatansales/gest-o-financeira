...
import { TrendingUp, TrendingDown, CreditCard, Wallet, DollarSign, CheckCircle, PieChart, BarChart3, Calendar, Target, Activity, Shield, AlertTriangle, Plus, Menu } from 'lucide-react';
...
interface FinancialOverviewDisplayProps {
  accounts: Account[];
  cards: Card[];
...
const FinancialOverviewDisplay: React.FC<FinancialOverviewDisplayProps> = ({
  accounts,
  cards,
  transactions,
  goals,
  ...
  const getLayoutClass = () => {
    switch (settings.layout) {
      case 'compact':
        return 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 lg:gap-4';
      case 'modern':
        return 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 lg:gap-8';
      case 'classic':
        return 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6';
      case 'minimal':
        return 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 lg:gap-3';
      default:
        return 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 lg:gap-6';
    }
  };
  ...
  const getCardClass = () => {
    switch (settings.layout) {
      case 'compact':
        return 'p-3 lg:p-4 rounded-lg shadow-md';
      case 'modern':
        return 'p-4 lg:p-8 rounded-2xl lg:rounded-3xl shadow-xl lg:shadow-2xl transform hover:scale-105';
      case 'classic':
        return 'p-4 lg:p-6 rounded-lg border-2 border-gray-200 shadow-sm';
      case 'minimal':
        return 'p-2 lg:p-3 rounded border border-gray-300';
      default:
  ...
  return (
    <div className="space-y-4 lg:space-y-8">
      ...
      <div className={`grid ${getLayoutClass()}`}>
        <div className={`bg-gradient-to-br from-blue-500 to-blue-600 text-white ${getCardClass()}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-blue-100 text-xs lg:text-sm font-medium">{t('overview.totalBalance')}</p>
              <p className="text-lg lg:text-2xl font-bold">{formatCurrency(summary.totalBalance)}</p>
            </div>
            <Wallet className="h-6 w-6 lg:h-10 lg:w-10 text-blue-200" />
          </div>
        </div>

        <div className={`bg-gradient-to-br from-green-500 to-green-600 text-white ${getCardClass()}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-green-100 text-xs lg:text-sm font-medium">{t('overview.monthlyIncome')}</p>
              <p className="text-lg lg:text-2xl font-bold">{formatCurrency(summary.monthlyIncome)}</p>
            </div>
            <TrendingUp className="h-6 w-6 lg:h-10 lg:w-10 text-green-200" />
          </div>
        </div>

        <div className={`bg-gradient-to-br from-red-500 to-red-600 text-white ${getCardClass()}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-red-100 text-xs lg:text-sm font-medium">{t('overview.monthlyExpenses')}</p>
              <p className="text-lg lg:text-2xl font-bold">{formatCurrency(summary.monthlyExpenses)}</p>
            </div>
            <TrendingDown className="h-6 w-6 lg:h-10 lg:w-10 text-red-200" />
          </div>
        </div>
        ...
        <div className={`bg-gradient-to-br from-purple-500 to-purple-600 text-white ${getCardClass()}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-100 text-xs lg:text-sm font-medium">{t('overview.totalReceived')}</p>
              <p className="text-lg lg:text-2xl font-bold">{formatCurrency(summary.totalReceived)}</p>
            </div>
            <CheckCircle className="h-6 w-6 lg:h-10 lg:w-10 text-purple-200" />
          </div>
        </div>
        ...
        <div className={`bg-gradient-to-br from-orange-500 to-orange-600 text-white ${getCardClass()}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-orange-100 text-xs lg:text-sm font-medium">{t('overview.totalPaid')}</p>
              <p className="text-lg lg:text-2xl font-bold">{formatCurrency(summary.totalPaid)}</p>
            </div>
            <DollarSign className="h-6 w-6 lg:h-10 lg:w-10 text-orange-200" />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
        ...
        <div className="bg-white rounded-xl lg:rounded-2xl shadow-lg p-4 lg:p-6 card-hover">
          <div className="flex items-center gap-3 mb-4">
            <div className="bg-gradient-to-r from-green-500 to-emerald-600 p-2 lg:p-3 rounded-full">
              <Target className="h-4 w-4 lg:h-6 lg:w-6 text-white" />
            </div>
            <div>
              <h3 className="text-base lg:text-lg font-bold text-gray-900">Minhas Metas</h3>
              <p className="text-gray-600 text-xs lg:text-sm">Progresso dos objetivos</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl lg:rounded-2xl shadow-lg p-4 lg:p-6 card-hover">
          <div className="flex items-center gap-3 mb-4">
            <div className="bg-gradient-to-r from-orange-500 to-red-600 p-2 lg:p-3 rounded-full">
              <Shield className="h-4 w-4 lg:h-6 lg:w-6 text-white" />
            </div>
            <div>
              <h3 className="text-base lg:text-lg font-bold text-gray-900">Meus Limites</h3>
              <p className="text-gray-600 text-xs lg:text-sm">Controle de gastos</p>
            </div>
          </div>
          <div className="space-y-3">
            <div className="flex justify-between items-center text-sm lg:text-base">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 lg:col-span-2 xl:col-span-1">
          <div className="flex items-center gap-3 mb-6">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className={`${annualIncome - annualExpenses >= 0 ? 'bg-gradient-to-br from-blue-500 to-blue-600' : 'bg-gradient-to-br from-orange-500 to-orange-600'} text-white p-6 rounded-xl`}>
            <div className="flex items-center justify-between mb-4">
...
export const FinancialModule: React.FC = () => {
  ...
  const [showCardInvoiceModal, setShowCardInvoiceModal] = useState(false);
  const [selectedCard, setSelectedCard] = useState<Card | null>(null);
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);

  return (
    <>
      <FinancialOverviewDisplay
        accounts={accounts}
        cards={cards}
      ...
      <button
        ...
        className="fixed bottom-20 right-4 lg:hidden bg-gradient-to-r from-blue-600 to-purple-600 text-white p-4 rounded-full shadow-2xl hover:shadow-3xl transform hover:scale-110 transition-all duration-300 z-40 touch-target"
        aria-label="Novo Lançamento"
      >
        <Plus className="h-6 w-6" />
      </button>

      {/* Mobile Menu Button for accessing other sections */}
      <button
        onClick={() => setIsSidebarOpen(true)}
        className="fixed bottom-20 left-4 lg:hidden bg-gradient-to-r from-gray-600 to-gray-700 text-white p-3 rounded-full shadow-xl hover:shadow-2xl transform hover:scale-110 transition-all duration-300 z-40 touch-target"
        aria-label="Menu"
      >
      ...
      <CardInvoiceModal
        isOpen={showCardInvoiceModal}
        onClose={() => {
          setShowCardInvoiceModal(false);
